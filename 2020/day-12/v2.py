...
def anyTrue(g: Iterable[bool]) -> bool:
...
# c   ns   ew
dirDict: Mapping[str, Sequence[int]] = {
    'N':  [-1,  0],
    'E':  [0, +1],
    'S':  [+1,  0],
    'W':  [0, -1],
...
def getAnswer_1(data: str) -> int:
    aoeu: Sequence[str] = data.strip().split("\n")
    dd = [I(d[0], int(d[1])) for d in aoeu]
